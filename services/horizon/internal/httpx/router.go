...
func (r *Router) addRoutes(config *RouterConfig, rateLimiter *throttled.HTTPRateLimiter, ledgerState *ledger.State) {
	...
	r.Group(func(r chi.Router) {
		r.Use(historyMiddleware)
		r.Method(http.MethodGet, "/claimable_balances/{claimable_balance_id:[[:xdigit:]]+}/operations", streamableHistoryPageHandler(ledgerState, actions.GetOperationsHandler{
			LedgerState:  ledgerState,
			OnlyPayments: false,
		}, streamHandler))
		r.Method(http.MethodGet, "/claimable_balances/{claimable_balance_id:[[:xdigit:]]+}/transactions", streamableHistoryPageHandler(ledgerState, actions.GetTransactionsHandler{LedgerState: ledgerState}, streamHandler))
	})
